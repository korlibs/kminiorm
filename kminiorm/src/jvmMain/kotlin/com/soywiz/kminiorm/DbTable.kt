...
abstract class AbstractDbTable<T : DbTableBaseElement> : DbTable<T> {
    ...
    override val dummyInstance: T by lazy { typerForClass.type(null) }
}
...
interface DbTable<T : DbTableBaseElement> {
    ...
    val dummyInstance: T
    fun <R> getProperty(first: KProperty<R>): KProperty1<T, R> = typerForClass.getProperty(first)
    ...
    // R
    suspend fun findFlowPartial(skip: Long? = null, limit: Long? = null, fields: List<KProperty1<T, *>>? = null, sorted: List<Pair<KProperty1<T, *>, Int>>? = null, query: DbQueryBuilder<T>.(T) -> DbQuery<T> = { everything }): Flow<Partial<T>>
    suspend fun findFlow(skip: Long? = null, limit: Long? = null, fields: List<KProperty1<T, *>>? = null, sorted: List<Pair<KProperty1<T, *>, Int>>? = null, query: DbQueryBuilder<T>.(T) -> DbQuery<T> = { everything }): Flow<T> = findFlowPartial(skip, limit, fields, sorted, query)
            .map { bindInstance(typerForClass.type(it.data)) }

    suspend fun find(skip: Long? = null, limit: Long? = null, fields: List<KProperty1<T, *>>? = null, sorted: List<Pair<KProperty1<T, *>, Int>>? = null, query: DbQueryBuilder<T>.(T) -> DbQuery<T> = { everything }): List<T> = findFlow(skip, limit, fields, sorted, query).toList()
    suspend fun findAll(skip: Long? = null, limit: Long? = null): List<T> = find(skip = skip, limit = limit)
    suspend fun findOne(query: DbQueryBuilder<T>.(T) -> DbQuery<T> = { everything }): T? = find(query = query, limit = 1).firstOrNull()
    suspend fun findChunked(skip: Long? = null, limit: Long? = null, fields: List<KProperty1<T, *>>? = null, sorted: List<Pair<KProperty1<T, *>, Int>>? = null, chunkSize: Int = 16, query: DbQueryBuilder<T>.(T) -> DbQuery<T> = { everything }): Flow<T> {
        val table = this
        return flow {
            var offset = skip ?: 0L
            var totalEmitted = 0L
    ...
    suspend fun count(query: DbQueryBuilder<T>.(T) -> DbQuery<T> = { everything }): Long = this.find(query = query).size.toLong()
    suspend fun <R> countGrouped(groupedBy: KProperty1<T, R>, query: DbQueryBuilder<T>.(T) -> DbQuery<T> = { everything }): Map<R, Long> =
            this.find(query = query).groupBy { groupedBy.get(it) }.map { it.key to it.value.size.toLong() }.toMap()

    // U
    suspend fun update(set: Partial<T>? = null, increment: Partial<T>? = null, limit: Long? = null, query: DbQueryBuilder<T>.(T) -> DbQuery<T>): Long
    ...
    // D
    suspend fun delete(limit: Long? = null, query: DbQueryBuilder<T>.(T) -> DbQuery<T>): Long
...
suspend fun <T : DbTableBaseElement> DbTable<T>.findOrCreate(query: DbQueryBuilder<T>.(T) -> DbQuery<T> = { everything }, build: () -> T): T = findOne(query) ?: build().also { insert(it) }
...
suspend fun <T : DbBaseModel> DbTable<T>.updateSimple(query: DbQueryBuilder<T>.(T) -> DbQuery<T>, build: UpdateSimpleBuilder<T>.() -> Unit): Long {
